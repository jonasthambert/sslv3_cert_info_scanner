#!/usr/bin/perl
# sslv3_banner_scanner.pl
# Most c0de stolen from sslv3_scanner.pl by vortextube
# Better parsing of cert info added
#
    ...
    foreach my $host (@$hosts) {
        ...
        foreach my $protocol (@$protocols) {
            ...
            if ($@) {
                ...
                $test_command_result = '';
                print "$host,unknown,timeout.\n";
            }

            if ($test_command_result =~ /CONNECTED/ && $test_command_result !~ /no peer certificate available/) {
		my $common_name = $test_command_result;
		my $issuer = $test_command_result;
		my $search = "subject";
		...
		if ($issuer =~ /issuer=/) {
		    $issuer = "empty\n";
                ...
                if ($test_command_result =~ /ssl handshake failure/) {
                    $test_result = 0;
                    print "$host,$protocol,unsupported\n";
                } else {
                    $test_result = 1;
                    print "$host,$protocol,,supported,$common_name,$issuer";
                }
            }
